...
This seems to fail for no apparent reason. Something about
matplotlib's object for 3d scatter plots simply doesn't let you
change the color of points after the fact.
"""
import int_gen
...
X = datasets.noisy_sphere(50)

# append a few random other things.
X = np.vstack([X, 2+0.5*X])

Y = -5 + generators_proto2.noisy_circles(10)
...
ax = []
ax.append( fig.add_axes([0.04,0.04,0.44,0.92]) )
ax.append( fig.add_axes([0.51,0.04,0.44,0.92], projection='3d') )
#ax.append( fig.add_axes(122, projection='3d'))
...
data_scatter = ax[1].scatter(X[:,0], X[:,1], X[:,2], c=[default_col])
